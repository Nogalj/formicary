...
- **Adding or swapping a mob sound**, or `runData` dying with `Found invalid sound events:
  ...
  against the filesystem; a `SoundType.EVENT` redirect is checked against the registry --
  which is why the scaffolding survived having no audio at all. Since round 8 the worker,
  soldier and queen have REAL files, generated by `assets-src\sounds.py` from the recordings
  in `assets-src\audio-src\`; everything else is still a redirect. Never hand-edit the
  three generated folders) -> `docs/SOUNDS.md`
